...
def test_cache_functionality():
    ...
    try:
        ...
        cached = db._get_from_cache("test_key")
        test_result("Zapis i odczyt z cache", 
                   cached is not None and len(cached) == 3)
        ...
        if cached is not None:
            ...
            test_result("Cache zwraca kopie (nie referencje)",
                       original['col1'].tolist() == [1, 2, 3])
        else:
        ...
        db.clear_cache("key1")
        test_result("Czyszczenie konkretnego klucza",
                   db._get_from_cache("key1") is None and 
                   db._get_from_cache("key2") is not None)
        
        # Test 1.5: Clear all cache
        db.clear_cache()
        test_result("Czyszczenie calego cache",
                   db._get_from_cache("key2") is None)
        ...
        test_result("TTL wygasanie cache",
                   immediate is not None and expired is None)
        ...
        test_result("Parametr use_cache dziala",
...
def test_index_recommendations():
    ...
    try:
        ...
        test_result("check_and_create_indexes zwraca slownik",
                   isinstance(result, dict))
        
        # Test 3.2: Result contains expected keys
        expected_keys = {'existing', 'missing', 'create_sql'}
        test_result("Wynik zawiera oczekiwane klucze",
                   expected_keys.issubset(result.keys()))
        
        # Test 3.3: existing is a list
        test_result("'existing' jest lista",
                   isinstance(result.get('existing'), list))
        ...
        if missing:
            ...
            has_required_fields = all(
                key in first_missing 
                for key in ['name', 'table', 'columns', 'sql', 'reason']
            )
            test_result("'missing' zawiera wymagane pola", has_required_fields)
        else:
            test_result("'missing' zawiera wymagane pola", True, 
                       "Brak brakujacych indeksow - wszystkie istnieja")
        
        # Test 3.5: create_sql is a list of strings
        create_sql = result.get('create_sql', [])
        test_result("'create_sql' jest lista stringow",
                   isinstance(create_sql, list) and 
                   all(isinstance(s, str) for s in create_sql))
...
def test_cache_with_real_queries():
    ...
    try:
        ...
        test_result("get_historical_data - pierwsze wywolanie",
                   not df1.empty,
        ...
        test_result("get_historical_data - z cache",
                   len(df1) == len(df2),
                   f"Czas: {cached_call_time:.3f}s")
        ...
        if first_call_time > 0.01:  # Only if first call was measurable
            ...
            test_result("Przyspieszenie dzieki cache",
                       cached_call_time < first_call_time,
                       f"Przyspieszenie: {speedup:.1f}x")
        else:
            test_result("Przyspieszenie dzieki cache", True, 
                       "Zbyt szybkie zapytanie do zmierzenia")
        ...
        test_result("get_current_stock cache",
        ...
        test_result("get_products_with_technology cache",
                   len(df_tech1) == len(df_tech2))
        
        # Test 4.6: Diagnostics stats after multiple queries
        stats = db.get_diagnostics_stats()
        test_result("Statystyki diagnostyczne po zapytaniach",
                   stats.get('total_queries', 0) >= 3,
                   f"Zapytan: {stats.get('total_queries', 0)}, "
                   f"Sredni czas: {stats.get('avg_duration', 0):.3f}s")
...
def test_bom_with_technology_id():
    ...
    try:
        ...
        test_result("get_bom_with_stock bez technology_id",
                   df_bom_default is not None)
        ...
        if not df_tech_id.empty:
            ...
            test_result("get_bom_with_stock z technology_id",
                       df_bom_specific is not None,
                       f"Technology ID: {tech_id}")
            
            # Test 5.4: Results should be same or subset
            if not df_bom_default.empty and not df_bom_specific.empty:
                test_result("Wyniki z technology_id sa poprawne",
                           len(df_bom_specific) <= len(df_bom_default) or
                           len(df_bom_specific) > 0)
        else:
            test_result("get_bom_with_stock z technology_id", True,
                       "Brak technology ID do testowania")
