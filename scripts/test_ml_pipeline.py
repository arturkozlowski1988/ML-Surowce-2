...
def test_prediction_viewmodel(results: TestResults, df: pd.DataFrame) -> bool:
    ...
    try:
        from src.viewmodels.prediction_viewmodel import PredictionViewModel, ModelType
        from src.preprocessing import prepare_time_series, fill_missing_weeks
        from src.forecasting import Forecaster
        ...
        if vm.prediction_state.available_products:
            ...
            for model_type in [ModelType.BASELINE, ModelType.RANDOM_FOREST]:
                ...
                if result is None or result.error:
                    results.add_fail(f"ViewModel train_model ({model_type.value})", 
                ...
                results.add_pass(f"ViewModel train_model ({model_type.value})", 
        ...
        if vm.prediction_state.model_results:
            ...
            results.add_pass("ViewModel get_combined_forecast_data", 
                            f"{len(combined_df)} rows, types: {list(types)}")
...
def test_analysis_viewmodel(results: TestResults, df: pd.DataFrame) -> bool:
    ...
    try:
        ...
        results.add_pass("AnalysisViewModel load_all_data", 
        ...
        if vm.analysis_state.summary:
            ...
            results.add_pass("AnalysisViewModel Summary", 
        ...
        if vm.analysis_state.df_historical is not None and not vm.analysis_state.df_historical.empty:
            ...
            if vm.analysis_state.df_filtered is not None:
                results.add_pass("AnalysisViewModel apply_date_filter", 
                                f"Filtered to {len(vm.analysis_state.df_filtered)} rows")
            else:
...
def test_model_accuracy(results: TestResults) -> bool:
    ...
    try:
        ...
        if error_pct > 5:  # Allow 5% error
            results.add_fail("Accuracy Test - Baseline", 
                            f"High error: {error_pct:.2f}% (predicted {avg_pred:.2f} vs expected ~{base_value})")
            return False
        
        results.add_pass("Accuracy Test - Baseline", 
                        f"Error: {error_pct:.2f}% (predicted {avg_pred:.2f} vs base {base_value})")
        ...
        if not predictions_rf.empty:
            ...
            if error_pct_rf < 15:  # More lenient for ML model
                results.add_pass("Accuracy Test - Random Forest", 
                                f"Error: {error_pct_rf:.2f}% (predicted {avg_pred_rf:.2f})")
            else:
                results.add_fail("Accuracy Test - Random Forest", 
                                f"High error: {error_pct_rf:.2f}%")
                return False
