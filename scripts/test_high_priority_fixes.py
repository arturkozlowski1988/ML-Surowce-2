...
def test_database_schema():
    ...
    try:
        ...
        test_result("CDN.Towary dostepna", not df1.empty, 
                   "Tabela CDN.Towary powinna zawierac dane")
        
        # Test 1.2: CDN.TwrZasoby table exists
        query2 = "SELECT TOP 1 TwZ_TwrId, TwZ_Ilosc FROM CDN.TwrZasoby"
        df2 = db.execute_query(query2)
        test_result("CDN.TwrZasoby dostepna", not df2.empty, 
                   "Tabela CDN.TwrZasoby powinna zawierac dane")
        ...
        test_result("dbo.CtiZlecenieNag dostepna", not df3.empty,
                   "Tabela dbo.CtiZlecenieNag powinna zawierac dane")
        ...
        test_result("dbo.CtiTechnolNag dostepna", not df4.empty,
                   "Tabela dbo.CtiTechnolNag powinna zawierac dane")
        ...
        test_result("JOIN CDN.Towary <-> dbo.CtiZlecenieElem", not df5.empty,
        ...
        test_result("get_current_stock() poprawny schemat", not df_stock.empty,
        ...
        if not df_tech.empty:
            ...
            test_result("get_bom_with_stock() poprawny schemat", 
                       df_bom is not None,  # May be empty but should not fail
                       "Metoda nie powinna rzucac bledu 'Invalid object name'")
        else:
            test_result("get_bom_with_stock() poprawny schemat", True, 
                       "Pominieto - brak produktow z technologia")
...
def test_empty_data_validation():
    ...
    for data, desc, expected_avg in test_cases:
        try:
            ...
            test_result(f"Kalkulacja sredniej ({desc})", passed,
                       f"Oczekiwano {expected_avg}, otrzymano {avg_consumption}")
        except Exception as e:
    ...
    try:
        ...
        if df_hist.empty:
            ...
            if not last_4_weeks or len(last_4_weeks) < 2:
                avg_consumption = last_4_weeks[0] if last_4_weeks else 0
                test_result("Walidacja z prawdziwymi danymi (malo danych)", True,
                           f"Poprawnie obsluzono {len(last_4_weeks)} rekordow")
            else:
                avg_consumption = sum(last_4_weeks) / len(last_4_weeks)
                test_result("Walidacja z prawdziwymi danymi (dosc danych)", 
                           avg_consumption > 0,
...
def test_database_methods():
    ...
    try:
        ...
        df_hist = db.get_historical_data()
        test_result("get_historical_data()", isinstance(df_hist, pd.DataFrame),
                   f"Zwrocono {len(df_hist)} rekordow")
        
        # Test 4.3: get_current_stock
        df_stock = db.get_current_stock()
        test_result("get_current_stock()", isinstance(df_stock, pd.DataFrame),
                   f"Zwrocono {len(df_stock)} produktow")
        
        # Test 4.4: get_products_with_technology
        df_tech = db.get_products_with_technology()
        test_result("get_products_with_technology()", isinstance(df_tech, pd.DataFrame),
        ...
        if not df_tech.empty:
            test_prod_id = int(df_tech['FinalProductId'].iloc[0])
            df_bom = db.get_bom_with_stock(test_prod_id)
            test_result("get_bom_with_stock()", isinstance(df_bom, pd.DataFrame),
        ...
        if not df_stock.empty:
            ...
            test_result("get_product_usage_stats()", isinstance(df_usage, pd.DataFrame),
