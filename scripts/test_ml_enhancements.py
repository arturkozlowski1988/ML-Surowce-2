...
def generate_test_data(n_weeks: int = 52) -> pd.DataFrame:
    ...
    for week in range(n_weeks):
        ...
        rows.append({"TowarId": 1, "Year": year, "Week": week_num, "Date": date, "Quantity": qty})
...
def test_ml_config(results: TestResults):
    ...
    try:
        from src.ml_config import MLConfig, get_model_config, load_config, save_config
...
def test_lstm_forecaster(results: TestResults):
    ...
    try:
        ...
        results.add_pass(
            "LSTM Forecaster", f"{len(predictions)} predictions, avg={predictions['Predicted_Qty'].mean():.2f}"
        )
...
def test_cross_validation(results: TestResults):
    ...
    try:
        ...
        cv_metrics = forecaster.cross_validate(df, product_id=1, model_type="rf", n_folds=3, test_size=4)
        ...
        results.add_pass(
            "Cross-validation",
            f"MAPE={cv_metrics['mape']:.2f}% (±{cv_metrics['mape_std']:.2f}), " f"RMSE={cv_metrics['rmse']:.2f}",
        )
