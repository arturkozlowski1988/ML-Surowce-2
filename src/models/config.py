...
def get_validated_config() -> AppConfig:
    ...
    # Load AI config
    config.ai = AIConfig(
        gemini_api_key=os.getenv("GEMINI_API_KEY"), ollama_host=os.getenv("OLLAMA_HOST", "http://localhost:11434")
    )
...
def get_safe_display_config(config: AppConfig) -> dict:
    ...
    result = {
        "database": None,
        "ai": {"gemini_configured": config.ai.gemini_api_key is not None, "ollama_host": config.ai.ollama_host},
        "debug": config.debug,
    }
