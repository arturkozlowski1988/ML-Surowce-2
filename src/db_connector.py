...
class QueryDiagnostics:
    ...
    def log_query(self, query_name: str, duration: float, row_count: int):
        ...
        self.query_times.append(
            {"query": query_name, "duration": duration, "rows": row_count, "timestamp": time.time()}
        )
...
class DatabaseConnector:
    ...
    def _create_db_engine(self):
        ...
        try:
            ...
            logger.info("Database engine created with pool: size=5, max_overflow=10, recycle=3600s")
            return engine
    ...
    def get_bom_with_stock(
        self, final_product_id: int, technology_id: int = None, warehouse_ids: list = None
    ) -> pd.DataFrame:
        """
    ...
    def check_and_create_indexes(self) -> dict:
        ...
        result = {"existing": [], "missing": [], "create_sql": []}
    ...
    def get_bom_with_delivery_info(
        self, final_product_id: int, technology_id: int = None, warehouse_ids: list = None
    ) -> pd.DataFrame:
        """
        ...
        df = self.execute_query(query, params=params, query_name=f"get_bom_with_delivery_info({final_product_id})")

        if not df.empty:
            # Use fallback lead time if no CtiDelivery data
            df["DeliveryTime_Days"] = df.apply(
                lambda row: row["DeliveryTime_Days"]
                if pd.notna(row["DeliveryTime_Days"]) and row["DeliveryTime_Days"] > 0
    ...
    def get_product_substitutes(
        self, product_id: int = None, technology_id: int = None, use_cache: bool = True
    ) -> pd.DataFrame:
        """
        ...
        df = self.execute_query(query, params=params if params else None, query_name="get_product_substitutes")
    ...
    def get_shortage_analysis_cti(self, warehouse_id: int = None, department_id: int = None) -> pd.DataFrame:
        """
    ...
    def get_production_resources(self, department_id: int = None, use_cache: bool = True) -> pd.DataFrame:
        """
        ...
        df = self.execute_query(query, params=params if params else None, query_name="get_production_resources")
    ...
    def get_production_status(self, order_id: int = None, use_cache: bool = True) -> pd.DataFrame:
        ...
        df = self.execute_query(query, params=params if params else None, query_name="get_production_status")
    ...
    def compare_with_cti_shortages(self, calculated_shortages: pd.DataFrame, warehouse_id: int = None) -> dict:
        """
        ...
        result = {"new_shortages": [], "already_in_cti": [], "cti_resolved": [], "summary": ""}
        ...
        calc_products = set(
            calculated_shortages.get("IngredientId", calculated_shortages.get("ProductId", [])).tolist()
        )
    ...
    def get_smart_substitutes(
        self, product_id: int, required_qty: float = 0, warehouse_ids: list = None
    ) -> pd.DataFrame:
        """
        ...
        df = self.execute_query(
            query, params={"product_id": product_id}, query_name=f"get_smart_substitutes({product_id})"
        )
    ...
    def get_production_dashboard_stats(self, date_from: str = None, date_to: str = None) -> dict:
        """
    ...
    def get_completions_history(self, order_id: int = None, limit: int = 100) -> pd.DataFrame:
        """
        ...
        return self.execute_query(query, params=params if params else None, query_name="get_completions_history")
    ...
    def get_product_cti_attributes(self, product_id: int = None, doc_type: int = 1) -> pd.DataFrame:
        """
        ...
        return self.execute_query(query, params=params, query_name="get_product_cti_attributes")
