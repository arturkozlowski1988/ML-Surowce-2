...
class LocalLLMEngine:
    ...
    def _initialize(self) -> bool:
        ...
        try:
            ...
            self.llm = Llama(
                model_path=str(model_file), n_ctx=self.n_ctx, n_threads=self.n_threads, verbose=self.verbose
            )
    ...
    def generate(
        self, prompt: str, max_tokens: int = 256, temperature: float = 0.7, stop: Optional[list[str]] = None
    ) -> str:
        """
        ...
        try:
            output = self.llm(prompt, max_tokens=max_tokens, temperature=temperature, stop=stop, echo=False)
