...
class SmartAlerts:
    ...
    def get_critical_shortages(
        ...
        usage_stats = df_hist.groupby('TowarId').agg({
        ...
        df['DaysOfStock'] = df.apply(
            ...
            if row['AvgWeeklyUsage'] > 0 else float('inf'),
            axis=1
        )
        ...
        df['DaysOfStock'] = df['DaysOfStock'].apply(
        ...
        return df[['Code', 'Name', 'StockLevel', 'AvgWeeklyUsage', 
                   'DaysOfStock', 'Status', 'Priority']]
