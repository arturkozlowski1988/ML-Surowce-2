...
class AuditService:
    ...
    def _save_log(self):
        ...
        try:
            ...
            data = {"entries": [asdict(e) for e in self._entries], "last_updated": datetime.now().isoformat()}
            with open(self.log_file, "w", encoding="utf-8") as f:
                json.dump(data, f, indent=2, ensure_ascii=False)
        except Exception as e:
            logger.error(f"Error saving audit log: {e}")

    def log(self, username: str, action: str, details: str = "", module: str = "", ip_address: str = ""):
        """
        Logs an audit entry.
    ...
    def get_entries(
        self, limit: int = 100, username: str = None, action_filter: str = None, days_back: int = None
    ) -> list[AuditEntry]:
        """
    ...
    def clear_old_entries(self, days_to_keep: int = 90):
        ...
        with self._lock:
            ...
            self._entries = [e for e in self._entries if datetime.fromisoformat(e.timestamp) >= cutoff]
            self._save_log()
