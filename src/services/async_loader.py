...
class AsyncDataLoader:
    ...
    def __init__(self):
        ...
        if AsyncDataLoader._executor is None:
            AsyncDataLoader._executor = ThreadPoolExecutor(
                max_workers=self._max_workers, thread_name_prefix="async_loader_"
            )
    ...
    def load_async(
        self, task_id: str, loader_fn: Callable[[], Any], force_reload: bool = False, cache_ttl: float = 300.0
    ) -> LoadResult:
        """
        ...
        if future_key in st.session_state:
            ...
            if future.done():
                ...
                try:
                    ...
                    data = future.result()
                    result = LoadResult(state=LoadingState.COMPLETED, data=data, duration_ms=duration_ms)
                    logger.info(f"Async load completed for {task_id} in {duration_ms:.0f}ms")
                except Exception as e:
                    result = LoadResult(state=LoadingState.ERROR, error=str(e))
    ...
    def load_sync_with_progress(
        self, task_id: str, loader_fn: Callable[[], Any], progress_text: str = "Ładowanie danych..."
    ) -> Any:
        """
    ...
    def clear_cache(self, task_id: Optional[str] = None):
        ...
        if task_id:
            ...
            keys_to_delete = [k for k in st.session_state.keys() if k.startswith(self._state_prefix)]
