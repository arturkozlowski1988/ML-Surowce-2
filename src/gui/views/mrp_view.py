...
def render_simulation_tab(
    ...
    if st.button("🚀 Uruchom Symulację", type="primary", key="mrp_simulate"):
        with st.spinner("Analizuję BOM i stany magazynowe..."):
            result = mrp.simulate_production(
        ...
        bom_df = result['bom']
        if not bom_df.empty:
            display_df = bom_df[['IngredientCode', 'IngredientName', 
                               'QuantityRequired', 'CurrentStock', 
                               'Shortage', 'Status']].copy()
            
            display_df.columns = ['Kod', 'Nazwa', 'Potrzeba', 'Stan', 'Różnica', 'Status']
            ...
            # Export shortages
            shortages_for_export = mrp.calculate_shortages(bom_df, target_quantity)
            if not shortages_for_export.empty:
                csv = shortages_for_export.to_csv(index=False)
                st.download_button(
                    "📥 Eksportuj braki do CSV",
                    csv,
                    f"braki_{selected_product_id}_{target_quantity}.csv",
                    "text/csv"
                )
