...
def render_mrp_view(db, product_map: dict[int, str], sorted_product_ids: list[int], warehouse_ids: list[int] = None):
    """
...
def render_simulation_tab(
    db, mrp, product_map: dict[int, str], sorted_product_ids: list[int], warehouse_ids: list[int] = None
):
    ...
    tech_product_map = {row["FinalProductId"]: f"{row['Name']} ({row['Code']})" for _, row in df_tech.iterrows()}
    ...
    with col2:
        target_quantity = st.number_input(
            "Ilość do wyprodukowania", min_value=1, max_value=100000, value=100, step=10, key="mrp_quantity"
        )
    ...
    if run_sim or analyze_ai:
        with st.spinner("Analizuję BOM, stany, dostawy i zamienniki..."):
            ...
            if analyze_ai:
                ...
                llm_result = mrp.analyze_with_llm(
                    product_id=selected_product_id, quantity=target_quantity, warehouse_ids=warehouse_ids
                )
        ...
        if not analyze_ai:  # If analyze_ai is true, we already showed markdown
            ...
            with col2:
                st.metric(
                    "Max. możliwa",
                    f"{result['max_producible']:.0f} szt.",
                    delta=f"{result['max_producible'] - target_quantity:.0f}"
                    if result["max_producible"] < target_quantity
                    else None,
                    delta_color="inverse",
                )
            ...
            if result["limiting_factor"] and not result["can_produce"]:
                ...
                **Czynnik ograniczający (bottleneck):**
                ...
                if lf.get("ingredient_id"):
                    subs = db.get_smart_substitutes(
                        lf["ingredient_id"], lf["quantity_required"] - lf["current_stock"], warehouse_ids
                    )
                    if not subs.empty:
            ...
            if not bom_df.empty:
                ...
                st.download_button(
                    "📥 Eksportuj braki do CSV", csv, f"braki_{selected_product_id}_{target_quantity}.csv", "text/csv"
                )
...
def render_alerts_tab(alerts, warehouse_ids: list[int] = None):
    ...
    status_filter = st.multiselect(
        "Filtruj wg statusu", options=["KRYTYCZNY", "NISKI"], default=["KRYTYCZNY", "NISKI"], key="alert_status_filter"
    )
    ...
    display_columns = ["Code", "Name", "StockLevel", "AvgWeeklyUsage", "DaysOfStock", "Status"]

    styled_df = (
        filtered_df[display_columns]
        .style.map(color_alert_status, subset=["Status"])
        .format({"StockLevel": "{:.1f}", "AvgWeeklyUsage": "{:.2f}"})
    )
    ...
    st.download_button("📥 Eksportuj alerty do CSV", csv, "alerty_magazynowe.csv", "text/csv")
...
def render_cti_dashboard(db):
    ...
    try:
        ...
        with col1:
            st.metric("📦 Aktywne Zlecenia", stats["orders"]["total"], help="Łączna liczba zleceń w okresie")
        with col2:
            st.metric(
                "⚠️ Dokumenty Braków", stats["shortages"]["active_docs"], f"{stats['shortages']['total_items']} pozycji"
            )
        with col3:
            st.metric("🛠️ Technologie", stats["technologies"]["active"], f"z {stats['technologies']['total']} total")
        with col4:
            st.metric("⚙️ Zasoby", stats["resources"]["total"])
