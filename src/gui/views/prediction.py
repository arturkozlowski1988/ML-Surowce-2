...
# Import ViewModels
from src.viewmodels import PredictionViewModel, ModelType, ModelResult
...
def render_prediction_view(
    ...
    chart_df = vm.get_combined_forecast_data(
        selected_product, model_type,
    ...
    with col1:
        ...
        * Średnio należy przygotować ok. **{avg_forecast:.1f}** jednostek tygodniowo {mae_context}.
        ...
        if 'forecast_change_pct' in result.metrics:
            ...
            st.markdown(f"**Trend:** {icon} {desc} zapotrzebowania o **{abs(change):.1f}%** względem ostatnich tygodni.")
